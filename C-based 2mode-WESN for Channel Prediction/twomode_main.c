...
//         if((g_start_index == g_T-g_B) && (WESN_DONE != 1)){ //All data has been sent
//         	double *NMSE_data = (double*)malloc(g_B * sizeof(double));

//         	for(int i = 0; i < g_T; i = (i+1)*g_B){ //double check condition //should iterate through each window size (0, 10, ... 390) if T_MAX==400
// 				NMSE_data = channel_pred_sys(&g_tensor[i * TRAIL_ELEMS]);//found in channel_pred_sys.c
// 				if (NMSE_data == NULL) return;

// 				//add function NMSE_send_data
// 				NMSE_send_data(NMSE_data, g_B);
//         	}
//         	free(NMSE_data);
